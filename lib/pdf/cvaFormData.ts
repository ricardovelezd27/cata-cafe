...
export const CVA_TEXT: Record<Locale, Record<string, string>> = {
  es: {
    ...
    // Official-form furniture
    methodology: "Evaluación del valor del café",
    formDescriptive: "Formulario descriptivo",
    formAffective: "Formulario afectivo",
    formCombined: "Formulario combinado",
    name: "Nombre",
    sampleNo: "Muestra n.º",
    roastLevel: "Nivel de tueste",
    impressionOfQuality: "Impresión de calidad",
    selectUpToFive: "Seleccione hasta cinco",
    selectUpToTwo: "Seleccione hasta dos",
    selectAllApply: "Seleccione los que apliquen",
    mainTastesTitle: "Gustos básicos (2)",
    defectIfAny: "Defecto (si lo hay)",
    partExtrinsic: "Parte 3 — Evaluación extrínseca",
    attribution:
      "Formato basado en el SCA Coffee Value Assessment v1.1 (sept. 2023). Documento derivado generado por Cata Café; no es un formulario oficial de la SCA.",
    // 9-point impression-of-quality scale
    q1: "Extremadamente baja",
    q2: "Muy baja",
    q3: "Moderadamente baja",
    q4: "Ligeramente baja",
    q5: "Ni alta ni baja",
    q6: "Ligeramente alta",
    q7: "Moderadamente alta",
    q8: "Muy alta",
    q9: "Extremadamente alta",
    // Short forms for the affective grid's narrow label column — the full
    // names wrap to two lines and break row alignment across sample columns.
  ...
  en: {
    ...
    // Official-form furniture
    methodology: "Coffee Value Assessment",
    formDescriptive: "Descriptive Form",
    formAffective: "Affective Form",
    formCombined: "Combined Form",
    name: "Name",
    sampleNo: "Sample No.",
    roastLevel: "Roast level",
    impressionOfQuality: "Impression of quality",
    selectUpToFive: "Select up to five",
    selectUpToTwo: "Select up to two",
    selectAllApply: "Select all that apply",
    mainTastesTitle: "Main tastes (2)",
    defectIfAny: "Defect (if any)",
    partExtrinsic: "Part 3 — Extrinsic assessment",
    attribution:
      "Layout based on the SCA Coffee Value Assessment v1.1 (Sept 2023). Derived document generated by Cata Café; not an official SCA form.",
    // 9-point impression-of-quality scale
    q1: "Extremely low",
    q2: "Very low",
    q3: "Moderately low",
    q4: "Slightly low",
    q5: "Neither high nor low",
    q6: "Slightly high",
    q7: "Moderately high",
    q8: "Very high",
    q9: "Extremely high",
...
// ─── Official-form sheet model ──────────────────────────────────────────────
//
// The paper CVA form does NOT repeat the descriptor list per section. It has
// exactly two shared CATA boxes — Fragrance+Aroma (Box A) and Flavor+Aftertaste
// (Box B) — each rendered as 9 category rows with their sub-options inline in
// parentheses, e.g. "FRUITY (BERRY  DRIED FRUIT  CITRUS FRUIT)". Our 9 L1 wheel
// groups map onto those 9 rows one-for-one and the L2 subgroups fill the
// parentheses, so the app's full vocabulary lands in the official footprint.
// L3 leaves + qualifying notes collapse onto the box's single notes line.
...
export type DescriptiveSheet = {
  intensity: Record<StageId, number | null>;
  /** Box A — Fragrance + Aroma, orthonasal. */
  boxA: FlavorGroupRow[];
  boxANotes: string[];
  /** Box B — Flavor + Aftertaste, retronasal. */
  boxB: FlavorGroupRow[];
  boxBNotes: string[];
  acidity: CataGroupRow[];
  sweetness: CataGroupRow[];
  mouthfeel: CataGroupRow[];
  /** Free-text `<stage>_notas` per section, for the ruled notes lines. */
  freeNotes: Record<StageId, string>;
};

/** Merge the selections + qualifying notes of two stages into one CATA box. */
function mergeStages(
  d: D,
  stages: StageId[]
): { selected: string[]; qualNotes: Record<string, string[]> } {
  const selected: string[] = [];
  const qualNotes: Record<string, string[]> = {};
  for (const stage of stages) {
    selected.push(...arr(d, `${stage}_desc`));
    const notes = d[`${stage}_desc_notes`] as Record<string, string[]> | undefined;
    if (!notes) continue;
    for (const [nodeId, terms] of Object.entries(notes)) {
      if (!Array.isArray(terms)) continue;
      qualNotes[nodeId] = [...(qualNotes[nodeId] ?? []), ...terms];
    }
  }
  return { selected, qualNotes };
}

/** Every group's notes-line entries, flattened to one box-level line. */
function boxNotes(groups: FlavorGroupRow[]): string[] {
  return dedupe(groups.flatMap((g) => g.notesLine));
}
...
export function buildDescriptiveSheet(d: D, locale: Locale): DescriptiveSheet {
  const a = mergeStages(d, ["fragancia", "aroma"]);
  const b = mergeStages(d, ["sabor", "sabor_residual"]);
  const boxA = buildFlavorGroups(a.selected, a.qualNotes, locale);
  const boxB = buildFlavorGroups(b.selected, b.qualNotes, locale);

  const cata = (stage: "acidez" | "dulzor" | "sensacion") =>
    buildCataGroups(
      stage,
      arr(d, `${stage}_desc`),
      (d[`${stage}_desc_notes`] as Record<string, string[]> | undefined) ?? {},
      locale
    );

  return {
    intensity: Object.fromEntries(
      ALL_STAGES.map((s) => [s, num(d, `${s}_int`)])
    ) as Record<StageId, number | null>,
    boxA,
    boxANotes: boxNotes(boxA),
    boxB,
    boxBNotes: boxNotes(boxB),
    acidity: cata("acidez"),
    sweetness: cata("dulzor"),
    mouthfeel: cata("sensacion"),
    freeNotes: Object.fromEntries(
      ALL_STAGES.map((s) => [s, str(d, `${s}_notas`)])
...
export type CvaSampleSheet = {
  ...
  /** Official paper-form view of the same descriptive data (shared CATA boxes). */
  descriptive: DescriptiveSheet;
...
export function buildCvaFormData(input: CvaFormInput): CvaSampleSheet {
  ...
  return {
    ...
    format,
    descriptorStages,
    descriptive: buildDescriptiveSheet(desc, locale),
    basicTastes,
    affectiveRows,
    cups,
    score,
